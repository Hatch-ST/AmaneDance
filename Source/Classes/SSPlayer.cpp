...
SSPlayer::SSPlayer(void)
	...
	, m_ssPlayerColor( ccc3(0xff,0xff,0xff) )
...
void SSPlayer::setAnimation(const SSData* ssData, SSImageList* imageList)
{
	...
	m_loopCount = 0;
	//追加
	m_numFrames = dataHandle->getNumFrames();
...
//追加
int SSPlayer::getNumFrames() const
{
	return m_numFrames;
}

//追加
void SSPlayer::setColor(cocos2d::ccColor3B color)
{
	m_ssPlayerColor = color;
	m_colorChangeFlag = true;
}

//追加
void SSPlayer::setColorFlag(bool changed)
{
	m_colorChangeFlag = changed;
}
...
void SSPlayer::setFrame(int frameNo)
{
	...
	for (size_t i = 0; i < numParts; i++)
	{
		...
		//追加 色が変更されていたら変える
		if(m_colorChangeFlag){
			sprite->setColor(m_ssPlayerColor);
		}
